...
public class RenderUtil {
    ...
    public static void drawBoundingBox (AxisAlignedBB bb, double width, JColor color) {
        Tessellator tessellator = Tessellator.getInstance();
        BufferBuilder bufferbuilder = tessellator.getBuffer();
    ...
    public static void createColorPre(JColor color, boolean isPlayer) {
        mc.getRenderManager().setRenderShadow(false);
        mc.getRenderManager().setRenderOutlines(false);
        GlStateManager.pushMatrix();
        GlStateManager.depthMask(true);
    ...
    public static void createWirePre(JColor color, int lineWidth, boolean isPlayer) {
        mc.getRenderManager().setRenderShadow(false);
        mc.getRenderManager().setRenderOutlines(false);
        GlStateManager.pushMatrix();
        GlStateManager.depthMask(true);
    ...
    public static void drawBoxWithDirection(AxisAlignedBB bb, JColor color, float rotation, double width, int mode) {
    ...
    public static void drawDirection(Points square, JColor color, double width) {
    ...
    public static void drawLineWidth(double posx, double posy, double posz, double posx2, double posy2, double posz2, JColor color, double width) {
        Tessellator tessellator = Tessellator.getInstance();
        BufferBuilder bufferbuilder = tessellator.getBuffer();
        GlStateManager.glLineWidth((float) width);
        color.glColor();
    ...
    public static void drawNametag (double x, double y, double z, String[] text, JColor color, int type) {
        ...
        if (type == 2) {
            ...
            for (int i = 0; i < text.length; i++) {
                double w= Horizon.cFont.getStringWidth(text[i])/2;
                if (w > width) {
                    width = w;
                }
            }
    ...
    public static void draw2DRect(int posX, int posY, int width, int height, int zHeight, JColor color) {
        Tessellator tessellator = Tessellator.getInstance();
        BufferBuilder bufferbuilder = tessellator.getBuffer();
